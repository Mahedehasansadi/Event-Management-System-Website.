...
if(isset($_POST['submit2']))
{
  ...
  $status=$_POST['status'];
  $remark=$_POST['remark'];
  $sql= "update tblpbooking set Status=:status,Remark=:remark where ID=:bid";
  ...
  if($query->execute()){
    ...
    echo "<script>window.location.href ='new_pbooking.php'</script>";
  }else{
...
<form class="forms-sample" method="post">
  <?php
  $eid2=$_POST['edit_id2'];
  $sql="SELECT * from  tblpbooking where ID=:eid2";
  $query = $dbh -> prepare($sql);
